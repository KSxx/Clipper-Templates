...
- `imdb-clipper.json` — creates a movie note from regular and localized IMDb title pages. Its distinct `IMDb – KSxx` name avoids collisions with generic templates. It stores extracted genres, directors, and cast as plain text, keeps IMDb and personal ratings separate, records the source URL, and places the cover and full visible plot in the note body for readable downstream processing.
