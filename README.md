...
- `wikipedia-clipper.json` — clips the current selection when text is selected and otherwise captures the detected article content. It removes Wikipedia edit-control lines from selections and trims full reference sections from complete articles while preserving the Clipper's native Markdown links. Wikipedia is stored as plain-text author, and organizational values are normal tags rather than wiki links.
