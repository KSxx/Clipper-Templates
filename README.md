...
- `wikipedia-clipper.json` — clips the current selection when text is selected and otherwise captures the detected article content. Selection-only clips discard orphaned HTML citation markers, while full-article clips retain Markdown footnotes and discard broken embeds that point to Wikipedia file-description pages. Wikipedia is stored as plain-text author, and organizational values are normal tags rather than wiki links.
