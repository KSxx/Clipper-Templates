...
- `wikipedia-clipper.json` — clips the current selection when text is selected and otherwise captures the detected article content. It removes Wikipedia edit-control lines, numbered citation markers, and full reference sections while preserving the Clipper's native Markdown links. Wikipedia is stored as plain-text author, and organizational values are normal tags rather than wiki links.
