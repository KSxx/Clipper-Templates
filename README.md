...
- `imdb-clipper.json` — creates a film or series note from regular and localized IMDb title pages. Its distinct `IMDb – KSxx` name avoids collisions with generic templates. File names include the release year to prevent collisions, film and series types are detected separately, and tracking parameters are removed from the stored source URL. It stores extracted genres, directors, and cast as plain text, keeps IMDb and personal ratings separate, and places the cover and full visible plot in the note body. A generated WerStreamt.es search link provides current streaming availability without storing provider data that can become stale.
