...
- `wikipedia-clipper.json` — clips the current selection when text is selected and otherwise captures the detected article content. It strips images and citation HTML before Markdown conversion, removes edit-control lines containing `action=edit`, and trims full-article references from the first numbered footnote definition onward. Wikipedia is stored as plain-text author, and organizational values are normal tags rather than wiki links.
