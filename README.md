...
- `wikipedia-clipper.json` — clips the current selection when text is selected and otherwise captures the detected article content. It strips images, citation markers, edit controls, and the full reference section before converting Wikipedia HTML to Markdown. Wikipedia is stored as plain-text author, and organizational values are normal tags rather than wiki links.
